 ...
 * 注册指定的外部日志。
 */
+ (void)registerExternalLog:(id<WMLogProtocol>)externalLog DEPRECATED_MSG_ATTRIBUTE("请直接使用 [WMLog registerExternalLog:] 方法");

/**
 * 获取已注册的外部日志。
 */
+ (id<WMLogProtocol>)registeredExternalLog DEPRECATED_MSG_ATTRIBUTE("请直接使用 [WMLog registeredExternalLog] 方法");
