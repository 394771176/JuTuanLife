...
//+ (void)load
//{
//    Method originalDecoderMethod = class_getInstanceMethod(self, @selector(valueForKey:));
//    Method modifiedDecoderMethod = class_getInstanceMethod(self, @selector(ff_valueForKey:));
//    method_exchangeImplementations(originalDecoderMethod, modifiedDecoderMethod);
//}
//
...
//{
//    NSArray *array = [DTClassObject propertyNames:self.class];
//    SEL sel = NSSelectorFromString(key);
//    DTLogTxt(@"%@\n%@ = %zd\n respond = %zd\n propertyNames : \n%@", NSStringFromClass(self.class), key, [array containsObject:key], [self respondsToSelector:sel], array);
//
//    return [self ff_valueForKey:key];
//}

//- (void)noRespondMethod:(NSString *)method
//{
//    if (APP_DEBUG) {
//        [DTPubUtil addBlockOnMainThread:^{
//            NSString *title = [NSString stringWithFormat:@"%s", _cmd];
//            NSString *msg = [NSString stringWithFormat:@"%@ => %@", self, method];
...
//    }
//    {
//        SEL sel = @selector(noRespondMethod:);
...
//    [self noRespondMethod:NSStringFromSelector(anInvocation.selector)];
//}

@end

@implementation NSTimer (Fix)
...
{
    return [NSTimer bk_timerWithTimeInterval:interval repeats:repeats usingBlock:block];
}

+ (NSTimer *)scheduledTimerWithTimeInterval:(NSTimeInterval)interval repeats:(BOOL)repeats block:(void (^)(NSTimer *timer))block
{
    return [NSTimer bk_scheduleTimerWithTimeInterval:interval repeats:repeats usingBlock:block];
}
